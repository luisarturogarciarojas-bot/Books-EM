    ...
    <>
      <AnimatePresence>
        {isVisible && (
          ...
          >
            <div className="relative overflow-hidden bg-[#FAF7F2] text-[#2C2420] rounded-2xl border border-[#D9C4B0] shadow-2xl shadow-[#3B2213]/20 backdrop-blur-md">
              {/* Brand Logo Header Image (Books EM Banner) */}
              <div className="relative w-full h-24 sm:h-28 bg-[#FAF6F1] border-b border-[#E8DFC8] overflow-hidden flex items-center justify-center">
                <img
                  src="/books-em-banner.jpg"
                  alt="Books EM Logo"
                  className="w-full h-full object-contain sm:object-cover object-center p-2"
                />

                {/* Top overlay badge */}
                <div className="absolute top-2.5 left-3 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#FAF7F2]/95 backdrop-blur-md text-[#5C3218] text-[10px] font-bold tracking-wide uppercase shadow-xs border border-[#E5DACB]">
                  <Sparkles className="w-3 h-3 text-[#D97706]" />
                  <span>Instalar Aplicación</span>
                </div>

                {/* Top close & timer */}
                <div className="absolute top-2.5 right-3 flex items-center gap-1.5 bg-[#FAF7F2]/95 backdrop-blur-md px-2 py-0.5 rounded-full border border-[#E5DACB] shadow-xs">
                  <span className="text-[10px] font-mono font-semibold text-[#8C5E3C]">
                    {secondsRemaining}s
                  </span>
                  <button
                    id="pwa-close-btn"
                    onClick={handleDismiss}
                    className="p-0.5 rounded-full text-[#786D65] hover:text-[#2C2420] transition-colors"
              ...
              {/* Main Content Body with App Icon Badge */}
              <div className="p-4 flex items-start gap-3.5">
                {/* App icon badge showing the installed app logo */}
                <div className="w-12 h-12 rounded-xl bg-[#FAF6F1] border border-[#D9C4B0] p-1 shadow-sm shrink-0 flex items-center justify-center overflow-hidden">
                  <img
                    src="/pwa-192x192.png"
                    alt="Books EM Icono"
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-serif-title font-bold text-sm text-[#3B2213] leading-snug">
                    {installSuccess
                      ...
                      : '¿Deseas instalar Books EM en tu dispositivo?'}
                  </h4>
                  <p className="text-xs text-[#6B5A4E] mt-0.5 leading-relaxed">
                    {installSuccess
                      ? 'Ya puedes acceder directamente con este icono desde tu pantalla de inicio.'
                      : 'Acceso instantáneo con este logo en tu pantalla de inicio, catálogo y compras por WhatsApp.'}
                  </p>
                </div>
              </div>

              {/* Action Buttons */}
              <div className="flex items-center justify-end gap-2 px-4 pb-3.5 pt-0">
                <button
                  id="pwa-dismiss-btn"
                  onClick={handleDismiss}
                  className="px-3 py-1.5 text-xs font-medium text-[#786D65] hover:text-[#3B2213] hover:bg-[#EFE6DC] rounded-lg transition-colors cursor-pointer"
                >
                  Quizás luego
                </button>
                ...
                {!installSuccess ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-[#5C3218] hover:bg-[#43230F] active:scale-[0.98] text-white text-xs font-semibold rounded-lg shadow-sm transition-all cursor-pointer"
                  >
        ...
        {showIOSModal && (
          ...
          >
            ...
            >
              ...
              {/* App Icon preview */}
              <div className="flex items-center gap-3.5 mb-4 p-3 rounded-xl bg-[#FAF6F1] border border-[#E5DACB]">
                <div className="w-14 h-14 rounded-xl bg-[#FAF6F1] border border-[#D9C4B0] p-1 shadow-sm shrink-0 flex items-center justify-center overflow-hidden">
                  <img
                    src="/pwa-192x192.png"
                    alt="Books EM Icono de App"
                    className="w-full h-full object-contain rounded-lg"
                  />
                </div>
                <div className="min-w-0 flex-1">
                  <h4 className="font-serif-title text-base font-bold text-[#3B2213]">Books EM</h4>
                  <p className="text-xs text-[#786D65]">Este será el icono oficial en tu dispositivo</p>
                  <span className="inline-block mt-0.5 text-[10px] text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.2 rounded font-medium">PWA Lista para instalar</span>
                </div>
              </div>
              ...
              <p className="text-xs text-[#6B5A4E] mt-1 leading-relaxed">
                {isIOS
                  ? 'Sigue estos sencillos pasos en Safari para agregar Books EM con este logo a tu pantalla de inicio:'
                  : 'Añade Books EM a tu pantalla principal siguiendo estos pasos desde tu navegador:'}
              </p>
